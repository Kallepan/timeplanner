...
describe('WorkdayAPIService', () => {
  ...
  it('should fetch workday details', async () => {
    ...
    service
      .getDetailWorkday(departmentName, date, workplace, timeslot)
      .subscribe((workday) => {
        expect(workday).toEqual(mockWorkday);
      });

    const req = httpController.expectOne(
      `${constants.APIS.PLANNER}/workday/detail?department=${departmentName}&date=${date}&workplace=${workplace}&timeslot=${timeslot}`,
    );
  ...
  it('should fetch workdays', () => {
    ...
    const req = httpController.expectOne(
      `${constants.APIS.PLANNER}/workday?department=${departmentName}&date=${date}`,
    );
  ...
  it('should unassign person from workday', () => {
    ...
    service
      .unassignPerson(departmentName, date, workplace, timeslot, personId)
      .subscribe((response) => {
        expect(response).toEqual(mockResponse);
      });

    const req = httpController.expectOne(
      `${constants.APIS.PLANNER}/workday/unassign`,
    );
  ...
  it('should assign person to workday', () => {
    ...
    service
      .assignPerson(departmentName, date, workplace, timeslot, personId)
      .subscribe((response) => {
        expect(response).toEqual(mockResponse);
      });

    const req = httpController.expectOne(
      `${constants.APIS.PLANNER}/workday/assign`,
    );
    expect(req.request.method).toEqual('POST');
    expect(req.request.withCredentials).toEqual(true);
    expect(req.request.headers.get('Content-Type')).toEqual('application/json');
